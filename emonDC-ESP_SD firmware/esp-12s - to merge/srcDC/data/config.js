...
if ((status.datalogfile!=0) && (status.datalogfile!="undefined")  ){
    document.getElementById("datalogfile").value = status.datalogfilename;
  }
...
// Event: Download file
// -----------------------------------------------------------------------
document.getElementById(“download-datalog”).addEventListener("click", function(e) {
    var datafile = {
      user: document.getElementById(“datalogfile”).value
    }
    document.getElementById("download-datalog").innerHTML = “Downloading…”;
    var r = new XMLHttpRequest();
    r.open("POST", “filedownload”, true);
    r.setRequestHeader("Content-type","application/x-www-form-urlencoded");
    r.send("&user="+admin.user+"&pass="+admin.pass);
    r.onreadystatechange = function () {
      console.log(admin);
